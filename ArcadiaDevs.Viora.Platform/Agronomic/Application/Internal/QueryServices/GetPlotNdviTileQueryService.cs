    ...
    IAgroMonitoringImageryService imageryService) : IGetPlotNdviTileQueryService
{
    public async Task<Result<byte[], Error>> HandleAsync(GetPlotNdviTileQuery query, CancellationToken cancellationToken = default)
    {
        ...
        if (plot == null)
        {
            return new Result<byte[], Error>.Failure(AgronomicErrors.PlotNotFound with { Message = $"Plot {query.PlotId} not found." });
        }

        if (plot.OwnerUserId != query.UserId)
        {
            return new Result<byte[], Error>.Failure(AgronomicErrors.PlotOwnership with { Message = $"Plot {query.PlotId} does not belong to user {query.UserId}." });
        }

        var isLinked = await imageryService.IsPlotLinkedAsync(plot, cancellationToken);
        if (!isLinked)
        {
            return new Result<byte[], Error>.Failure(AgronomicErrors.PlotNotLinked);
        }
        ...
        {
            return new Result<byte[], Error>.Failure(AgronomicErrors.TileNotFound);
        }
